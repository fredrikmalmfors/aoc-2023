...
inits = []
inits += [((y, -1), 0) for y in range(len(lines))]
inits += [((-1, x), 1) for x in range(len(lines[0]))]
inits += [((y, len(lines[0])), 2) for y in range(len(lines))]
inits += [((len(lines), x), 3) for x in range(len(lines[0]))]
...
for bb in inits:
    ...
    res = len(set(pos for pos, _ in VISITED)) - 1
    hoi.append(res)
